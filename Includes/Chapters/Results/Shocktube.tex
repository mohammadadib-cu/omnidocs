...
The IPR model of Omnisoot is used to simulate soot formation in shock-tubes. The measured pressure-time-history is imposed on the model for the simulation of each datapoint. Due to the large variability of kinetic mechanisms in the prediction of species, especially PAHs, for each dataset, we perform a systematic analysis of gas chemistry by comparing the predicted mole fraction of $\mathrm{CH_4}$, $\mathrm{C_2H_4}$, and $\mathrm{C_2H_2}$ with the measurements. Then, we compare the the measured $f_v$ considering the reported uncertainties with the maximum possible $f_v$ that can be achieved using each reaction mechanism. We also perform a parametric study for each inception model to find the set of inception ($\eta_{inc}$) and adsorption ($\eta_{ads}$) adjustment factors to minimize the error of prediction of $f_v$ and $d_p$ (when available) for the entire range of the studied $T_5$. This provide the range of carbon flux from gas to soot the enables accurate prediction of soot yield and morphology.
...
\begin{table}[H]
	...
	\begin{tabular}{lllllllll}
		\cline{2-9}
     	& \multicolumn{8}{c}{\textbf{Datapoints}}                       \\ \cline{2-9} 
...
The performance of reaction mechanisms are assessed by comparing the mole fraction of species captured by laser diagnostics, $\mathrm{CH_4}$, $\mathrm{C_2H_4}$, and $\mathrm{C_2H_2}$ with model predictions using Caltech~\citep{blanquart2009chemical}, KAUST~\cite{wang2013pah} and ABF~\citep{appel2000kinetic} mechanisms. We also introduce a new mechanism, K-FFCM2, built by combining FFCM2~\cite{ZDV2023} and KAUST mechanisms. FFCM2~\cite{ZDV2023} is a kinetic mechanism developed in Stanford by Prof. Hai Wang's group for the combustion of $\mathrm{C_{1-4}}$ hydrocarbons, and it is optimized, uncertainty-minimized, and validated against over 1000 combustion datasets using the polynomial chaos expansions method~\citep{sheen2011method}. However, this mechanism only includes $\mathrm{C_1}$-$\mathrm{C_4}$ species and excludes benzene and larger PAHs, so it cannot be used by Omnisoot. To  utilize the capabilities of this optimized mechanism, we extracted the $\mathrm{C_{5-24}}$ chemistry from KAUST and combined it with FFCM2 to build K-FFCM2. Some minor modifications were also made to the hybrid mechanism including reducing the rate constants of propargyl recombination reactions (i.e., $2\mathrm{C_3H_3}\rightarrow \mathrm{A1}$ and $2\mathrm{C_3H_3}\rightarrow \mathrm{A1^-}$) to improve the prediction of species.   

The analysis of simulation results (see Figs.~\ref{fig:shocktubest_30ch4_ch4_0}-\ref{fig:shocktubest_30ch4_ch4_0}) showed that the soot formation described by different PAH growth and particle dynamcis models has negligible impact on prediction the mole fraction of measured species in the studied temperature range. Therefore, the assessment of reaction mechanisms for prediction of major species is performed without soot to simply the analysis. Figs.~\ref{fig:shocktubest_30ch4_ch4_c2h2_c2h4} compares the mole fraction of $\mathrm{CH_4}$ (a-c), $\mathrm{C_2H_4}$ (d-c), $\mathrm{C_2H_2}$ (g-i) predicted using Caltech, KAUST, ABF, and K-FFCM2 with measurements by laser diagnostics. Note that only a subset of simulations are shown for brevity, the corresponds to datapoints (1), (4), (7) in Table~\ref{tab:shocktubest_CH4_30}. The results for the entire temperature range can be found in Figs.~\ref{fig:shocktubest_30ch4_nosoot_ch4}-\ref{fig:shocktubest_30ch4_nosoot_c2h2}. $\mathrm{CH_4}$ mole fraction starts with a sharp drop from 0.3 (30\%) at early residence times ($t\le0.1$~ms) and it is followed by a more gradual decrease after $t\ge0.25$~ms. The rate of initial fast decomposition increases with temperature. All mechanisms underpredict $\mathrm{CH_4}$ mole fraction considering the uncertainties of  the measurements, but ABF mechanism predictions are closer to the measurements than KAUST, Caltech, and K-FFCM2.

The mole fraction of $\mathrm{C_2H_4}$ starts with a quick jump after methane decomposition, reaches the maximum value and gradually decreases towards its final value.  The carbon flux analysis, performed for all the mechanisms, highlighted the major role of $\mathrm{C_2H_4}$ in the production of  $\mathrm{C_2H_2}$ via vinyl radical ($\mathrm{C_2H_3}$)
as shown in prior studies (Fig.~10 in~\citep{wang2023systematic}). KAUST and K-FFMC2 predict the largest and smallest $\mathrm{C_2H_4}$ mole fraction, and the peak is best captured by Caltech in the studied temperature range, respectively. At $\mathrm{T_5}=1861$K, the measured $\mathrm{C_2H_4}$ mole fraction consistently falls between the ABF and Caltech predictions up to 0.5ms, with this trend continuing to 1~ms. In $\mathrm{T_5}\ge2000$~K, the measurements reaches a plateau around 0.01 after 0.25~ms, but all mechanisms predict a continued decrease toward smaller values until 1~ms. 

All mechanisms predict the $\mathrm{C_2H_2}$ mole fraction within the uncertainty range of the measurements and capture its increasing trend over time. At $\mathrm{T_5}\ge2000$~K, the rapid initial rise becomes more evident. The nominal $\mathrm{C_2H_2}$ mole fraction is best reproduced by K-FFCM2, while ABF and KAUST remain close to the upper and lower bounds of the uncertainty range, respectively, across the entire $\mathrm{T_5}$ range. The Caltech mechanism exhibits stronger temperature dependence: it is similar to ABF at $\mathrm{T_5}=1861$~K, coincides with K-FFCM2 at $\mathrm{T_5}=2020$~K, and lies between K-FFCM2 and KAUST at $\mathrm{T_5}=2200$~K. 
 
 \begin{figure}[H]
 	\centering
 	\includegraphics[width=1\textwidth]{Figures/Results/Shocktube/30CH4/CH4_C2H4_C2H4.pdf}
 	\caption{The time history of mole fraction of $\mathrm{CH_4}$ (a-c), $\mathrm{C_2H_4}$ (d-f), $\mathrm{C_2H_2}$ (g-i) during 30\% $\mathrm{CH_4}$ pyrolysis at $\mathrm{T_5}=1861$, 2155, and 2375 K using Caltech, KAUST, ABF, and K-FFCM2 mechanisms compared with laser diagnostics data}
 	\label{fig:shocktubest_30ch4_ch4_c2h2_c2h4} 
 \end{figure}

As shown in Figure~\ref{fig:shocktubest_30ch4_A1_A2R5}, greater variability is observed among the kinetic mechanisms in predicting the mole fractions of benzene (A1) and naphthalene (A2R5) compared to $\mathrm{CH_4}$, $\mathrm{C_2H_4}$, and $\mathrm{C_2H_2}$. KAUST and ABF predict the highest and lowest A1 mole fractions, respectively, across the studied temperature range. As expected, the predictions of K-FFCM2 are lower than those of KAUST by a factor of 2–4, mainly due to the reduced rates of A1 production reactions in K-FFCM2. All mechanisms predict an increasing trend for A1, although K-FFCM2 reaches this trend only after 0.5ms. As discussed in Section\hl{******}, A2R5 is the dominant contributor to inception and PAH adsorption because its concentration is larger than that of other soot precursors considered in this study. Therefore, its mole fraction under no-soot conditions can be used to compare inception flux and yield expected from each mechanism. ABF remarkably predicts a low A2R5 mole fraction ($\le10^{-5}$) in all conditions. KAUST predicts the highest A2R5 mole fraction at $\mathrm{T_5}=1861$~K, but the trend changes for  $\mathrm{T_5}\ge2000$~K in which K-FFCM2 yield a higher A2R5 mole fraction.

 \begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{Figures/Results/Shocktube/30CH4/A1_A2R5.pdf}
	\caption{The time history of mole fraction of $\mathrm{A1}$ (a-c), $\mathrm{A2R5}$ (d-f) during 30\% $\mathrm{CH_4}$ pyrolysis at $\mathrm{T_5}=1861$, 2155, and 2375 K using Caltech, KAUST, ABF, and K-FFCM2 mechanisms compared with laser diagnostics data}
	\label{fig:shocktubest_30ch4_A1_A2R5} 
\end{figure}
