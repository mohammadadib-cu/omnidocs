...
\reaction[reac:dehyd_ebri]{
	PAH_j + H <-->[$k_{f,d_{j}}$][$k_{r,d_{j}}$] $\dot{\mathrm{PAH}}_j$ + H_2
}
...
\begin{equation}
	\omega_{dim_j} = k_{inc_{j}} [\mathrm{\cdot{PAH}}_j]^2,
	\label{eqn:wdim_ebri}
\end{equation}
...
\renewcommand{\arraystretch}{1.5}
\begin{table}
	\caption[Rate coefficients for the monomer de-/hydrogenation reaction of E-bridge formation in Arrhenius form $\mathrm{k=AT^n\cdot e^{-E/RT}}$.]{Rate coefficients for the monomer de-/hydrogenation reaction of E-bridge formation in Arrhenius form $\mathrm{k=AT^n\cdot e^{-E/RT}}$~\citep{frenklach2020mechanism}.}
	\label{tab:Ebridge}
	\centering
...
The comparison of key features of the implemented PAH growth models is provided in Table~\ref{tab:incsummary}. As shown in Fig.~\ref{fig:inceptempdep} the inception flux predicted by the Reactive Dimerization and E-Bridge Modified models increases exponentially with temperature (that is, decreases with its inverse), whereas the Irreversible Dimerization and Dimer Coalescence models show only about a factor-of-two increase over the range 500-2500~K.

\renewcommand{\arraystretch}{1.2}
\begin{table*}
	\caption{The summary of key features of PAH growth models.}
	\label{tab:incsummary}
	\centering
	\begin{tabular}{l l p{1.5cm} p{7.5cm}}
		\hline
		Name & Reversibility & Temperature dependence & The sequence of steps in inception mechanism \\
		\hline
		Irrev.Dim.~\cite{frenklach1991detailed}  & Irreversible & Weak, $\propto$$\sqrt{T}$ &
		{[PAH]$\xrightarrow[\text{}]{\text{collision}}$[Dim.]}
		\\
		Reac.Dim.~\citep{kholghy2018reactive} & Reversible & Strong, $\propto$$\:\mathrm{e}^{-E/(RT)}$  &
		{[PAH]$\xleftrightarrow[\text{}]{\text{collision}}$[Phy.Dim.]$\xrightarrow[\text{}]{\text{carbonization}}$[Chem.Dim]}
		\\
		Dim.Coal.~\citep{blanquart2009analyzing} & Irreversible & Weak, $\propto$$\sqrt{T}$ &
		{[PAH]$\xrightarrow[\text{}]{\text{collision}}$[Dim.]
			$\xrightarrow[\text{}]{\text{collision}}$[Tetra.]}
		\\
		EBri.Mod.~\citep{frenklach2020mechanism}  & Reversible & Strong, $\propto$$\:\mathrm{e}^{-E/(RT)}$ & 
		{[PAH]$\xleftrightarrow[\text{}]{\text{dehydrogenation}}$[Rad.]
			$\xrightarrow[\text{}]{\text{collision}}$[Chem.Dim.]}
		\\
		\hline
		\multicolumn{4}{l}{\footnotesize{Dim. denotes PAH dimers.}} \\
		\multicolumn{4}{l}{\footnotesize{Phy.Dim. and Chem.Dim. represent physically- and chemically-bonded dimers, respectively.}} \\
		\multicolumn{4}{l}{\footnotesize{Rad. and Tetra. refer to a PAH radical and a tetramer, respectively.}}
	\end{tabular}
\end{table*}

\begin{figure}[!t]
	\centering
	\includegraphics[width=0.65\textwidth]{Figures/Theory/incepmodels_T_dep.pdf}
	\caption{The temperature dependence of inception flux, $I_{inc}$, predicted using various PAH growth models. $I_{inc}$ of each model is normalized with its maximum to facilitate the comparison.}
	\label{fig:inceptempdep} 
\end{figure}