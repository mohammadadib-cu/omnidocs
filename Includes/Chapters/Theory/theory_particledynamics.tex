...
\begin{figure}[!t]
	\centering
	\includegraphics[width=0.8\textwidth]{Figures/Theory/Sectional.pdf}
	\caption[The illustration of sections in SPBM.]{The illustration of sections in SPBM. The mass of sections grows progressively by the scale factor of SF. Inception introduces new particles to the first section that propagate to the upper section via coagulation and surface growth and return to lower sections by oxidation. Carbon and hydrogen pass from gas to solid phase through inception and surface growth and goes back via oxidation.}
	\label{fig:sectional}
\end{figure}
...
%\subsubsection{Coagulation source terms}
...
\begin{equation}
	...
	\begin{alignedat}{2}
		...
		&\quad 
		+ I^{i}_{H_{tot},haca} + I^{i}_{H_{tot},ads},
		&\qquad& \text{if } 1 < i < n_{sec}
		\\
		&\frac{I^{i-1}_{C_{tot},haca}+I^{i-1}_{C_{tot},ads}}{C^{i}_{agg}-C^{i-1}_{agg}}H^{i-1}_{agg}
		+ I^{i}_{H_{tot},haca} + I^{i}_{H_{tot},ads},
		&\qquad& \text{if } i=n_{sec}
	\end{alignedat}
	\right.
