...
In the perfectly stirred reactor (PSR) model, gas enters with a known mass flow rate of $\dot{m_{in}}$, composition of $Y_{in}$ and temperature of $T_{in}$. Then, it instantaneously mixes and homogeneously reacts with the mixture resident inside the reactor. The reacting gas reaches a spatially uniform temperature and composition described by $T$, and $Y$. It is assumed that temperature, composition and soot properties of the outflow are the same as those inside the reactor. Figure~\ref{fig:psrcv} illustrates the schematics of PSR where ${\dot{m}_{in}}$ and $\mathrm{\dot{m}_{out}}$ refer to inflow and outflow gas mass flow rates only, respectively, and $V$ is the volume confined within the reactor boundaries that are assumed to be fixed. 

Steady-state solution to PSR is often of interest. In that case, the time derivative terms in governing equations become zero, so the system of ordinary differential equations are reduced to a set of coupled nonlinear algebraic equations. Solving this system of equations requires a sophisticated implementation to account for non-linearity and mulitple solutions. A practical alternative is simulating PSR as a time-dependent constant volume reactor with specified in- and outflow that is started from an initial state and advanced in time until the solution is converged. This approach has already be implemented and tested in Cantera~\citep{cantera} for a reacting gas with no soot formation. The mean residence time of gas mixture in the reactor is defined as the ratio of mass inside the reactor to the inflow mass flow rate:
...
Under no-soot conditions, the steady-state values of inlet and outlet mass flow rates become equal. However, the gas mixture loses mass to solid phase by soot formation, so the final value of ${\dot{m}_{out}}$ is less than that of $\mathrm{\dot{m}_{in}}$. 
%The pressure of reactor is assumed to stay constant during the process~\citep{kee2017chemically}. 
...
The conservation of mass equation can be used to determine PSR by considering the mass flux of in- and outflow, and the removal of mass due to soot generation as:
...
The outlet gas mass flow rate, $\dot{m}_{out}$ is calculated from inlet mass flow rate, $\dot{m}_{in}$ and a small correction determined by pressure differences as:
...
\begin{equation}
	...
	\dot{m}_{in}
	+ K_v (P_1 - P_2)
	\label{eqn:mdotoutpsr},
\end{equation}

\noindent where $K_v$ proportionality constant is set to $10^{-2}$ by default. Its value does not change the steady-state solution of the equations, but it controls transient solution and may help or hinder the convergence.
The density is determined by solving the continuity equation, and the pressure is calculated from ideal gas law using the composition from solving the species transport equations as:

\begin{equation}
	\frac{d Y_k}{d t}
	=
	\frac{\dot{m}_in}{\rho V (1-\varphi)}
	\left( Y^*_k-Y_k \right)+
...
\begin{table}
	...
	\begin{tabular}{l l l l l l}
		...
		\stepcounter{reaction}\thetag{\thereaction}\label{reac:haca3} & \ce{C_{soot\textdegree} + H -> C_{soot-H}} & f & $10^4$ & 0.734 & 719.68\\
